...
class GY87
{
	...
	bool begin(mpu6050_dps_t scale = MPU6050_SCALE_2000DPS, mpu6050_range_t range = MPU6050_RANGE_2G, int mpua = MPU6050_ADDRESS, int hmcla=HMC5883L_ADDRESS);

	void setClockSource(mpu6050_clockSource_t source);
	void setScale(mpu6050_dps_t scale);
	void setRangeAccel(mpu6050_range_t range);
	mpu6050_clockSource_t getClockSource(void);
	mpu6050_dps_t getScale(void);
	mpu6050_range_t getRangeAccel(void);
  ...
  int hmclAddress;
	...
	uint8_t fastRegister8(uint8_t addr,uint8_t reg);
	...
	void writeRegister16(uint8_t addr,uint8_t reg, int16_t value);

	bool readRegisterBit(uint8_t addr,uint8_t reg, uint8_t pos);
	void writeRegisterBit(uint8_t addr,uint8_t reg, uint8_t pos, bool state);
